	...
	ERC20VotesUpgradeable
{
  ...
  function gcred_amount() 
    internal 
    pure 
    returns(uint16[16] memory gcred) 
  {
  ...
  function _calcReward(address _address, uint _duration) 
    internal 
    returns(uint reward, uint _gcred_amount) 
  {
    ...
    uint16[16] memory getGcredAmount = gcred_amount();
    _gcred_amount = getGcredAmount[staker.interest];
  }
